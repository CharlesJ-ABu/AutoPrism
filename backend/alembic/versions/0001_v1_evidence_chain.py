"""Create the V1 schema and upgrade legacy L1 records with evidence metadata."""
...
def upgrade() -> None:
    bind = op.get_bind()
    # This first migration also serves as a baseline for empty local databases.
    Base.metadata.create_all(bind=bind)

    inspector = inspect(bind)
    raw_columns = {column["name"] for column in inspector.get_columns("raw_intelligence")}
    additions = {
    ...
    for constraint in inspector.get_unique_constraints("raw_intelligence"):
        if constraint.get("column_names") == ["source_url"] and constraint.get("name"):
            op.drop_constraint(
                constraint["name"], "raw_intelligence", type_="unique"
            )
    ...
    # Existing rows predate verifiable acquisition and must remain explicitly marked.
    op.execute(
