...
- M15 面板级可信地图：`trusted_map` UI DSL 只允许绑定含可执行
  `geo-scope-v1` 的冻结面板，并只读取该面板当前可重放的
  `trusted-insight-map-v1` 要素；桌面和移动端会从可信几何自动取景，且加载、错误、
  空状态和截断均明确可见。
...
同日 M15 面板级可信地图：全新隔离 PostgreSQL 从零迁移到 `0013`，完整后端
套件 73/73 通过，`alembic check` 无漂移；前端 typecheck/build 与生产依赖
0 漏洞审计通过。隔离非空面板地图、加载/错误/空状态、桌面/390px 自动取景、
选择反馈和真实移动端抽屉遮挡回归均通过，真实数据库仍未写入测试要素。
